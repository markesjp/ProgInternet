...
<main class="container py-4">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <textarea id="motivoStatus" class="form-control" rows="2" maxlength="255" placeholder="Ex.: aluno solicitou pausa / desistência / pendência documental..."></textarea>
  ...
  <script>
    ...
    tbody.addEventListener('click', async (e) => {
      ...
      if (action === 'edit') {
        // Pega detalhe (garante campos completos)
        try {
    ...
    function renderTable(list) {
      ...
      for (const a of list) {
        ...
        tr.innerHTML = `
          ...
          <td>${escapeHtml(a.data_matricula)}</td>
          <td class="text-end">
            <button class="btn btn-sm btn-outline-primary me-1" data-action="edit" data-id="${a.id}" title="Editar"><i class="bi bi-pencil-square"></i></button>
            ${String((a.status||'ATIVO')).toUpperCase() === 'INATIVO'
              ? `<button class="btn btn-sm btn-outline-success" data-action="activate" data-id="${a.id}" title="Ativar"><i class="bi bi-person-check"></i></button>`
              : `<button class="btn btn-sm btn-outline-warning" data-action="deactivate" data-id="${a.id}" title="Desativar"><i class="bi bi-person-x"></i></button>`
            }
          </td>
        `;
        tbody.appendChild(tr);
    ...
    tbody.addEventListener('click', async (e) => {
      ...
      if (action === 'deactivate' || action === 'activate') {
        ...
        if (action === 'deactivate') {
          warn.style.display = '';
          info.textContent = '';
          // Opcional: busca contagem de aulas futuras marcadas para informar no modal
          apiGet('future_count', { id: row.id }).then((r) => {
            if (r && typeof r.count !== 'undefined') {
              info.textContent = `Aulas futuras marcadas a desmarcar: ${r.count}`;
            }
          }).catch(() => {});
