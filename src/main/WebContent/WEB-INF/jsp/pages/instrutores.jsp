...
<script>
  ...
  const API_INSTRUTORES = BASE + '/instrutores';
  ...
  async function apiGet(op, params = {}) {
    const url = new URL(API_INSTRUTORES, window.location.origin);
  ...
  async function apiPost(op, body) {
    const res = await fetch(API_INSTRUTORES + '?format=json', {
      method: 'POST',
  ...
  function renderTable(list) {
    ...
    for (const i of list) {
      ...
      tr.innerHTML = `
        <td class="text-muted">${escapeHtml(i.id)}</td>
        <td>
          <div class="fw-semibold">${escapeHtml(i.nome)}</div>
          <div class="text-muted small">${escapeHtml(i.telefone || '')}</div>
        </td>
        <td>${escapeHtml(applyMaskCPF(i.cpf || ''))}</td>
        <td><span class="badge text-bg-light border">${escapeHtml(i.especialidade || '')}</span></td>
        <td>${escapeHtml(i.data_contratacao || '')}</td>
        <td class="text-end">
          <button class="btn btn-sm btn-outline-primary me-1" data-action="edit" data-id="${escapeHtml(i.id)}"><i class="bi bi-pencil-square"></i></button>
          <button class="btn btn-sm btn-outline-danger" data-action="delete" data-id="${escapeHtml(i.id)}"><i class="bi bi-trash"></i></button>
        </td>
      `;
  ...
  function applyFilter() {
    ...
    const filtered = rows.filter(r =>
      ...
      String(r.especialidade || '').toLowerCase().includes(q)
    );
  ...
  function openEdit(i) {
    ...
    fields.cpf.value = applyMaskCPF(i.cpf || '');
